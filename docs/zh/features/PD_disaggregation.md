...
**Prefill / Decode 分离**指将预填充与解码阶段调度到不同实例协同完成。部署配置与 KV 传输参数见 [PD 分离服务部署](../user_guide/service_deployment/pd_disaggregation_deployment.md)。
...
> Prefill 与 Decode **两端 `dispatch_profile` 必须一致**，且取值须与 connector 实际协同语义匹配，否则 Coordinator 仍无法配对（503）。字段说明见 [user_config 全量参数说明](../user_guide/service_deployment/config_reference.md#61-dispatch_profilepd-协同语义)。
