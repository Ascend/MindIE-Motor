...
| 配置项 | 类型 | 说明 |
|--------|------|------------------|
| context_budget_mode | string | 请求路由前按 prompt token 数将 `max_tokens` / `max_completion_tokens` 裁剪到模型剩余上下文。可选值：`off`（默认）或 `on`；开启 `on` 时，P/D 引擎配置必须提供 `model` 与 `max_model_len`。 |
| log_level | string | 日志级别。默认值：INFO<ul><li>DEBUG</li><li>INFO</li><li>WARNING</li><li>ERROR</li></ul> |
...
| 配置项 | 类型 | 说明 |
|--------|------|--------|
| enable_virtual_inference | bool | 虚推总开关，默认值：false。<br>取值为 `true` 时，在推理面 `/health` 正常后启动周期性虚推。<br>**仅允许在 ERROR 日志级别下开启**（`ASCEND_GLOBAL_LOG_LEVEL=3`，未配置默认 ERROR）；显式配置为非 ERROR 时 Engine Server 启动虚推前会关闭并打印 warning。 |
