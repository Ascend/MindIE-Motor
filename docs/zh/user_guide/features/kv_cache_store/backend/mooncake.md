...
- **多硬件适配**：支持 Ascend 950PR系列产品、Atlas 800I A3超节点服务器、Atlas 800I A2推理服务器等多种硬件平台。
...
| 硬件 | 依赖 | 说明 |
|------|------|------|
| Ascend 950PR系列产品 | <ul><li>HDK >= 25.6</li><li>mooncake >= v0.3.11</li><li>CANN >= 9.1.0</li></ul>  | 需根据实际通信协议配置 UBOE 或 UB 环境变量。 |
| Atlas 800I A3超节点服务器 | <ul><li>HDK >= 26.0 或（HDK >= 25.5 且 mooncake >= v0.3.11）</li><li>CANN >= 9.1.0</li><li>灵衢算力网络 >= 1.5</li></ul> | 推荐启用统一内存地址直传方案。 |
| Atlas 800I A2推理服务器 | <ul><li>HDK >= 25.5</li><li>CANN >= 9.1.0</li></ul> | 无需额外环境变量，通用必配项即可。 |
...
| 硬件 | 环境变量 | 说明 |
|------|-------|------|
| Ascend 950PR系列产品 | <ul><li>使用 UBOE 协议时：ASCEND_GLOBAL_RESOURCE_CONFIG={"comm_resource_config.protocol_desc":["uboe:device"]}</li><li>使用 UB 协议时：ASCEND_LOCAL_COMM_RES={"version":"1.3"}</li></ul> | UBOE / UB 二选一，按实际使用的通信协议配置。 |
...
| Atlas 800I A3超节点服务器 | ASCEND_BUFFER_POOL=4:8 | 当[表 1](#table001)依赖列软件版本不满足时，使用该方案。<br>配置 NPU Device 上用于聚合与 KV 传输的 buffer 个数与大小（例如 `4:8` 表示 4 个 8MB buffer）。 |
| Atlas 800I A2推理服务器 | — | 无需额外环境变量，通用必配项即可。 |
...
1. 在 user_config.json配置文件 中配置 Mooncake 后端字段 kv_cache_store_config，样例如下。
    ...
    **表 4** 参数说明
    ...
    >[!NOTE] 说明
    >可选配置 `target_job_id` 复用其他推理服务的 kv_store（值为目标服务的 `job_id`），详情请参见[多套服务共享kv_store](../README.md#table_Connector)。
...
下方示例 `connectors[0]` 为标准 attention 配置，混合 attention 须换为 `MooncakeHybridConnector`，详情请参见 [P/D传输Connector选型](../README.md#table_Connector)。
...
4. 使用部署脚本启动服务。
    ...
    >- store 进程的通信环境同样由 NodeManager 自动配置，无需手工干预：store 独占 HIXL comm_resource_config.listen_port=26666（写入 ranktable 的 device_port，与同卡引擎 worker 区分，避免 HCCL EI0014），HCCL socket 端口段相对引擎偏移（Atlas 800I A2推理服务器：HCCL_NPU_SOCKET_PORT_RANGE=16700-16800；Ascend 950PR系列产品：host socket 段 +2000），避免与引擎的 16666/RA socket 冲突（EI0020）。

<!-- npu="950" id1 -->
#### 场景三：Ascend 950PR系列产品额外配置

在 Ascend 950PR系列产品上使用 Mooncake 后端时，需确保 Pod 能访问宿主机侧 UB 相关网卡（ipourma*），任选以下方式之一。
...
2. 补充 Ascend 950PR系列产品 引擎环境变量。
   编辑 examples/deployer/startup/common.sh 中的 set_a5_engine_env（由 roles/engine.sh 在 Ascend 950PR系列产品 场景调用），补齐示例如下：
