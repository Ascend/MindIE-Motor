...
`enable-prefix-caching` 配置在 `motor_engine_union_config.engine_config` 中；Coordinator启动时会从 union 引擎段自动合并 `prefill_kv_event_config`（`endpoint`、`replay_endpoint`、`model_path` 等），无需手写 prefill 段配置。
