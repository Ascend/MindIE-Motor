    ...
    - PD分离场景端口配置示例
      ...
      ```json
      {
        "motor_deploy_config": {
          ...
          "single_p_instance_pod_num": 2,       // 请勿修改
          "single_d_instance_pod_num": 4        // 请勿修改
        },
    ...
    - PD 混部场景端口配置
      ...
      ```json
      {
        "motor_deploy_config": {
          ...
          "single_hybrid_instance_pod_num": 2,  // 请勿修改
...
**网络**：正文 `docker run` 已使用 `--net=host`，Atlas 800I A3 超节点服务器 场景继续使用 host 网络。
...
Atlas 800I A3 超节点服务器 启动示例片段（基于上述实例基础修改）：
