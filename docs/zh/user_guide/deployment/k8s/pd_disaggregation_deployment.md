...
- **注意**：虚推**仅允许在 ERROR 日志级别下开启**（`ASCEND_GLOBAL_LOG_LEVEL=3`，未配置默认即为 ERROR）。若引擎进程环境中显式配置为非 ERROR，Engine Server 启动虚推前会关闭虚推并打印 warning。该功能使用详情请参见 [虚推健康检查](../../features/sim_inference.md)。
