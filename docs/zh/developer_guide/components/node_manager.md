...
启用虚推时（见 [虚推健康探测](../../user_guide/features/sim_inference.md)），`NativeEngineService` 每个实例维护**单个**虚推 monitor，**仅对 vLLM** 初始绑定 DP0，并在 DP 缩容提交后迁移到新的 DP Master（`TargetIdentity` 不可变，含 instance/endpoint/host/port/engine_type；`VirtualInferenceSpec` 含 role/model/TLS/阈值/超时）。NodeManager 在 pull 时保留本地 endpoint 的候选 spec；收到携带新 master rank 的 finalize 后，master 所在节点安装新 monitor，其他节点清理旧 monitor。SGLang 不创建 Motor 虚推 monitor/worker：由自身在 `GET /health` 中执行生成式健康检查（拉起时强制 `SGLANG_ENABLE_HEALTH_ENDPOINT_GENERATION=true`），NodeManager/`ProcessSupervisor` 仅做现有 `/health` 心跳。`runtime_state(endpoint, instance_id)` 在 target 身份匹配且首次观察到原生 `/health` `READY` 后幂等启动 `VirtualInferenceWorker`（`motor/node_manager/core/services/native_engine/virtual_inference/`，由 Node Manager 直接执行，不依赖 Engine Server）：
