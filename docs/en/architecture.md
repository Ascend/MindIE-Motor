# MindIE Motor Architecture

## Introduction to MindIE Motor

**MindIE Motor** is a request scheduling framework oriented to distributed inference of large language models (LLMs), such as **PD disaggregation inference** (separation of the Prefill and Decode phases). Through an open and scalable inference service platform architecture, it connects downward to [vLLM-Ascend](https://github.com/vllm-project/vllm-ascend), aiming to meet the high-performance inference requirements of LLMs.

> [!NOTE]NOTE
> The original MindIE PyMotor repository has been renamed to MindIE Motor starting from version 3.1.0, and this naming will be used in subsequent releases. Its software positioning and core functionalities remain unchanged, and it remains compatible with the vLLM-Ascend inference engine.

### Core Capabilities

MindIE Motor primarily provides capabilities in the following two aspects:

- **PD disaggregation request scheduling**: distributes external customer requests to the Prefill/Decode instance with the lowest load, achieving **load balancing**.

- **Reliability, Availability, and Serviceability (RAS)**: enhances the **reliability, availability, and serviceability** of the PD disaggregation service.

## System Architecture

The interaction architecture of MindIE Motor and its peripheral components is shown in the following figure:

**Figure 1 MindIE Motor architecture**

![Architecture diagram](./imgs/MindIE_Motor_Architecture.png)

## Key Components and Modules

The core components of MindIE Motor are defined as follows:
...
- (Optional) **[CCAE](https://www.hiascend.com/software/ccae)**: visualized O&M platform, which integrates computing, storage, and network resources.
