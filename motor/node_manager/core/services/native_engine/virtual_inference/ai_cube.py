...
def _read_first_ai_cube_usage_from_watch(proc: subprocess.Popen) -> int:
    """Read raw stdout bytes until the first complete data row appears, then stop."""
    ...
    while time.monotonic() < deadline:
        ...
        while b"\n" in buffered:
            ...
            usage = _parse_usage_from_line(raw_line.decode(errors="replace"))
            if usage is not None:
                return usage

    if buffered:
        usage = _parse_usage_from_line(buffered.decode(errors="replace"))
        if usage is not None:
            return usage
