...
- 部署前必须把 `user_config.json` 中的 `image_name` 替换为实际可用的 UCM 运行镜像。镜像需同时包含当前分支构建的 Motor wheel、匹配的 vLLM/vllm-ascend，以及能够导入 `ucm.integration.vllm.ucm_connector` 的兼容 UCM 包。本仓库当前提供的基础镜像构建文件位于 `docker/mindie-motor-vllm/<版本-硬件-OS-架构>/Dockerfile`；UCM 运行镜像需按实际环境构建并发布后填写到示例中。
...
- **`type: "hostpath"`** —— 节点本地目录直挂，仅允许 `/data` 或 `/mnt` 目录及其子目录。**注意:hostPath 会绕过容器存储隔离，只能由可信集群管理员配置，生产集群还应通过准入策略限制。**hostPath 本身不跨节点共享——只有当该路径在每个节点都预先挂了同一共享文件系统(如各节点同路径挂同一 NFS 导出)时,P/D 才真正共享;纯本地目录会静默造成跨节点前缀缓存零命中。能用 `nfs`/`pvc` 就别用它。
...
|---|---|---|---|
| `type` | 是 | — | `"hostpath"` |
| `path` | 是 | — | 节点上的绝对目录,必须位于 `/data` 或 `/mnt` 下 |
| `mount_path` | 建议 | `/mnt/store-<i>` | 挂载点,需与 UCM `storage_backends` 一致 |
...
当同时使用 `motor_deploy_config.storage` 和内联 UCM 配置时，部署器会在生成 YAML 前校验每个 `ucm_connector_config.storage_backends` 都与某个已启用的 `storage[i].mount_path` 完全一致，避免 Pod 启动后才发现后端目录未挂载。使用 `UCM_CONFIG_FILE` 的文件配置无法由部署器读取，仍需由使用者保证其中路径与容器挂载一致。
