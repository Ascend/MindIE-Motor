...
* ``storage`` — a LIST of volume specs mounted into every engine pod. Each entry MUST declare
  its ``type``:
    ...
    - ``"hostpath"`` — a node-local hostPath (``path``) restricted to the operator-managed
      ``/data`` and ``/mnt`` roots. Cross-node sharing ONLY if that path is itself a shared
      filesystem mounted identically on every node (hostPath-over-NFS); a plain local dir
      silently breaks cross-node prefix-cache hits.
...
_ALLOWED_HOSTPATH_PREFIXES = ("/data", "/mnt")
...
def _validate_hostpath_entry(entry, index):
    ...
    _require_non_empty_string(entry, index, C.PATH)
    normalized_path = posixpath.normpath(entry[C.PATH].strip())
    allowed = any(
        normalized_path == prefix or normalized_path.startswith(f"{prefix}/") for prefix in _ALLOWED_HOSTPATH_PREFIXES
    )
    if not posixpath.isabs(normalized_path) or not allowed:
        raise ValueError(
            f"'{C.STORAGE}[{index}].{C.PATH}' = {entry[C.PATH]!r} must resolve to an "
            f"absolute path under one of {_ALLOWED_HOSTPATH_PREFIXES}. Arbitrary hostPath "
            "mounts can expose sensitive node files; use pvc/nfs for other storage roots."
        )
...
def _iter_ucm_connectors(user_config):
    """Yield UCM connector configs from every engine role that can carry one."""
    engine_keys = (
        C.MOTOR_ENGINE_ENCODE_CONFIG,
        C.MOTOR_ENGINE_PREFILL_CONFIG,
        C.MOTOR_ENGINE_DECODE_CONFIG,
        C.MOTOR_ENGINE_UNION_CONFIG,
    )
    for engine_key in engine_keys:
        engine_section = user_config.get(engine_key)
        if not isinstance(engine_section, dict):
            continue
        engine_config = engine_section.get(C.ENGINE_CONFIG)
        if not isinstance(engine_config, dict):
            continue
        kv_config = engine_config.get(C.KV_TRANSFER_CONFIG)
        if not isinstance(kv_config, dict):
            continue
        kv_config_path = f"{engine_key}.{C.ENGINE_CONFIG}.{C.KV_TRANSFER_CONFIG}"
        if kv_config.get(C.KV_CONNECTOR) == C.UCM_CONNECTOR:
            yield kv_config_path, kv_config
            continue
        if kv_config.get(C.KV_CONNECTOR) != C.MULTI_CONNECTOR:
            continue
        extra_config = kv_config.get(C.KV_CONNECTOR_EXTRA_CONFIG)
        connectors = extra_config.get(C.CONNECTORS) if isinstance(extra_config, dict) else None
        if not isinstance(connectors, list):
            continue
        for connector_index, connector in enumerate(connectors):
            if isinstance(connector, dict) and connector.get(C.KV_CONNECTOR) == C.UCM_CONNECTOR:
                yield (
                    f"{kv_config_path}.{C.KV_CONNECTOR_EXTRA_CONFIG}.{C.CONNECTORS}[{connector_index}]",
                    connector,
                )


def _normalize_ucm_storage_backends(value, config_path):
    """Normalize UCM's string (colon-separated) or JSON-list backend representation."""
    if isinstance(value, str):
        backends = [path.strip() for path in value.split(":") if path.strip()]
    elif isinstance(value, list) and all(isinstance(path, str) and path.strip() for path in value):
        backends = [path.strip() for path in value]
    else:
        raise ValueError(
            f"'{config_path}.{C.STORAGE_BACKENDS}' = {value!r} must be a non-empty path string "
            "(multiple paths separated by ':') or a non-empty list of path strings."
        )
    if not backends:
        raise ValueError(f"'{config_path}.{C.STORAGE_BACKENDS}' must contain at least one path.")
    return [posixpath.normpath(path) for path in backends]


def _validate_ucm_storage_backends(user_config, entries):
    """Fail before YAML generation when inline UCM paths are not mounted by storage entries.

    File-based UCM configs cannot be inspected safely from the deployer host and remain the
    UCM connector's responsibility. Validation is intentionally limited to inline configs
    when ``motor_deploy_config.storage`` is in use.
    """
    if not entries:
        return
    mount_paths = {posixpath.normpath(_entry_mount_path(entry, index)) for index, entry in enumerate(entries)}
    for connector_path, connector in _iter_ucm_connectors(user_config):
        extra_config = connector.get(C.KV_CONNECTOR_EXTRA_CONFIG)
        if not isinstance(extra_config, dict) or C.UCM_CONFIG_FILE in extra_config:
            continue
        ucm_connectors = extra_config.get(C.UCM_CONNECTORS)
        if not isinstance(ucm_connectors, list):
            continue
        for ucm_index, ucm_connector in enumerate(ucm_connectors):
            if not isinstance(ucm_connector, dict):
                continue
            ucm_config = ucm_connector.get(C.UCM_CONNECTOR_CONFIG)
            if not isinstance(ucm_config, dict) or C.STORAGE_BACKENDS not in ucm_config:
                continue
            config_path = (
                f"{connector_path}.{C.KV_CONNECTOR_EXTRA_CONFIG}."
                f"{C.UCM_CONNECTORS}[{ucm_index}].{C.UCM_CONNECTOR_CONFIG}"
            )
            backends = _normalize_ucm_storage_backends(ucm_config[C.STORAGE_BACKENDS], config_path)
            unmatched = [backend for backend in backends if backend not in mount_paths]
            if unmatched:
                raise ValueError(
                    f"'{config_path}.{C.STORAGE_BACKENDS}' contains {unmatched}, but enabled "
                    f"'{C.MOTOR_DEPLOY_CONFIG}.{C.STORAGE}' mount paths are {sorted(mount_paths)}. "
                    "Each inline UCM storage backend must exactly match an enabled storage mount_path."
                )
...
def get_storage_entries(user_config):
    ...
    _validate_ucm_storage_backends(user_config, entries)
    return entries
...
def _build_volume(entry, index):
    ...
    if entry_type == C.STORAGE_TYPE_HOSTPATH:
        host_path_source = {C.PATH: posixpath.normpath(entry[C.PATH].strip())}
...
def apply_storage_volumes(pod_spec, container, user_config, entries=None):
    ...
    for index, entry in enumerate(entries):
        ...
        expected_volume = _build_volume(entry, index)
        expected_mount = {C.NAME: name, C.MOUNT_PATH: mount_path}
        if entry.get(C.STORAGE_READ_ONLY):
            expected_mount[C.K8S_READ_ONLY] = True

        existing_volume = next((volume for volume in volumes if volume.get(C.NAME) == name), None)
        if existing_volume is not None and existing_volume != expected_volume:
            raise ValueError(
                f"Pod spec volume '{name}' conflicts with '{C.STORAGE}[{index}]': "
                f"existing={existing_volume!r}, expected={expected_volume!r}. Rename the "
                "template volume or remove the conflicting definition."
            )
        existing_mount = next((mount for mount in mounts if mount.get(C.NAME) == name), None)
        if existing_mount is not None and existing_mount != expected_mount:
            raise ValueError(
                f"Container volumeMount '{name}' conflicts with '{C.STORAGE}[{index}]': "
                f"existing={existing_mount!r}, expected={expected_mount!r}. Remove or align "
                "the template mount before enabling this storage entry."
            )

        if existing_volume is None:
            volumes.append(expected_volume)
        if existing_mount is None:
            mounts.append(expected_mount)
