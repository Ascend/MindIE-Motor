...
def test_standalone_ucm_connector_passes_through_in_union():
    """Union keeps the standalone UCM config unchanged instead of entering P/D processing."""
    endpoint_config = _make_endpoint_config(dp_size=1, tp_size=4)
    ucm_config = {
        "kv_connector": "UCMConnector",
        "kv_role": "kv_both",
    ...
    endpoint_config.deploy_config.engine_config.set("kv_transfer_config", ucm_config)

    config = VLLMConfig(endpoint_config=endpoint_config)
    config.initialize()

    assert config.kv_transfer_config is None
    assert config._flatten_config()["kv_transfer_config"] == ucm_config
...
def test_vllm_rejects_malformed_multi_connectors(extra_config):
    """Direct VLLMConfig callers receive a clear ValueError before connector indexing."""
    endpoint_config = _make_endpoint_config(dp_size=1, tp_size=4)
    endpoint_config.role = "prefill"
    kv_config = {"kv_connector": "MultiConnector"}
    if extra_config is not None:
        kv_config["kv_connector_extra_config"] = extra_config
    endpoint_config.deploy_config.engine_config.set("kv_transfer_config", kv_config)
