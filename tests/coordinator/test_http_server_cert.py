...
def create_other_ca():
    ...
    other_ca_cert = (
        x509.CertificateBuilder()
        .subject_name(other_ca_subject)
        .issuer_name(other_ca_subject)
...
def test_cert_util_validation(test_certificates):
    ...
    validation_result = CertUtil.validate_certificate_chain(
        ca_file=test_certs["ca_cert"], cert_file=test_certs["server_cert"], key_file=test_certs["server_key"]
    )
...
def test_create_ssl_context_no_client_cert(test_certificates):
    ...
    ssl_context = CertUtil.create_ssl_context_no_client_cert(
        cert_file=test_certs["server_cert"], key_file=test_certs["server_key"], ca_file=test_certs["ca_cert"]
    )
    assert ssl_context is not None, "SSL context should be created successfully"
    ...
    ssl_context = CertUtil.create_ssl_context_no_client_cert(
        cert_file=test_certs["server_cert"], key_file=test_certs["server_key"], ca_file=""
    )
...
def test_create_ssl_context_error_handling():
    ...
    empty_tls_config = TLSConfig(enable_tls=True, ca_file="", cert_file="", key_file="")
    ...
    ssl_context = CertUtil.create_ssl_context_no_client_cert(cert_file=None, key_file=None, ca_file=None)
    assert ssl_context is None, "None values should return None"

    # Test with empty key_file
    ssl_context = CertUtil.create_ssl_context_no_client_cert(cert_file="/nonexistent/cert.pem", key_file="", ca_file="")
    assert ssl_context is None, "Empty key_file should return None"

    # Test with non-existent certificate files
    ssl_context = CertUtil.create_ssl_context_no_client_cert(
        cert_file="/nonexistent/cert.pem", key_file="/nonexistent/key.pem", ca_file=""
    )
    assert ssl_context is None, "Non-existent certificate files should return None"
...
def test_cert_info_query(test_certificates):
    ...
    try:
        ...
        with open(invalid_cert_path, "w", encoding="utf-8") as f:
...
def test_validate_certificate_chain(test_certificates):
    ...
    result = CertUtil.validate_certificate_chain(
        ca_file="/nonexistent/ca.pem", cert_file="/nonexistent/cert.pem", key_file="/nonexistent/key.pem"
    )
    assert result is False, "Non-existent files should return False"

    result = CertUtil.validate_certificate_chain(ca_file="", cert_file="", key_file="")
...
def test_validate_revoke_list(test_certificates):
    ...
    try:
        ...
        with open(invalid_crl_path, "w", encoding="utf-8") as f:
...
@pytest.mark.parametrize(
    ("ca_crt_path", "plain_text"),
    [
        (None, None),
        ("", None),
        ("ca", None),
        (None, b""),
        (None, b"test_password"),
    ],
    ids=["no-ca", "empty-ca", "with-ca", "empty-password", "password-ignored"],
)
def test_validate_cert_and_key_success_paths(test_certificates, ca_crt_path, plain_text):
    """Cover validate_cert_and_key success paths without repeating full validation."""
    test_certs = test_certificates
    kwargs = {
    ...
    if ca_crt_path == "ca":
        kwargs["ca_crt_path"] = test_certs["ca_cert"]
    elif ca_crt_path is not None:
        kwargs["ca_crt_path"] = ca_crt_path
    if plain_text is not None:
        kwargs["plain_text"] = plain_text

    assert CertValidationUtil.validate_cert_and_key(**kwargs) is True


@pytest.mark.parametrize(
    ("server_crt_path", "server_key_path"),
    [
        (None, "/nonexistent/key.pem"),
        ("/nonexistent/cert.pem", None),
        ("", "/nonexistent/key.pem"),
        ("/nonexistent/cert.pem", ""),
        ("/nonexistent/server_cert.pem", "/nonexistent/server_key.pem"),
    ],
)
def test_validate_cert_and_key_rejects_invalid_paths(test_certificates, server_crt_path, server_key_path):
    assert (
        CertValidationUtil.validate_cert_and_key(
...
def test_validate_cert_and_key_rejects_empty_files():
    temp_dir = tempfile.mkdtemp()
    try:
        empty_cert_path = os.path.join(temp_dir, "empty_cert.pem")
        empty_key_path = os.path.join(temp_dir, "empty_key.pem")
        with open(empty_cert_path, "w", encoding="utf-8"):
            pass
        with open(empty_key_path, "w", encoding="utf-8"):
            pass
...
def test_validate_cert_and_key_rejects_invalid_formats():
    temp_dir = tempfile.mkdtemp()
    try:
        invalid_cert_path = os.path.join(temp_dir, "invalid_cert.pem")
        ...
        with open(invalid_key_path, "w", encoding="utf-8") as f:
            f.write(_INVALID_PKCS8_KEY_PEM)
        ...
        with open(valid_cert_path, "w", encoding="utf-8") as f:
...
def test_validate_cert_and_key_rejects_mismatched_key(test_certificates):
    test_certs = test_certificates
    temp_dir = tempfile.mkdtemp()
    try:
...
@pytest.mark.parametrize(
    "ca_setup",
    ["missing", "invalid-format", "mismatched", "empty-file"],
)
def test_validate_cert_and_key_rejects_ca_errors(test_certificates, ca_setup):
    test_certs = test_certificates
    if ca_setup == "missing":
        ...
        temp_dir = tempfile.mkdtemp()
        if ca_setup == "invalid-format":
            ca_crt_path = os.path.join(temp_dir, "invalid_ca.pem")
            with open(ca_crt_path, "w", encoding="utf-8") as f:
                f.write("This is not a valid CA certificate")
        elif ca_setup == "mismatched":
            _other_ca_key, other_ca_cert = create_other_ca()
            ca_crt_path = os.path.join(temp_dir, "other_ca.pem")
            with open(ca_crt_path, "wb") as f:
                f.write(other_ca_cert.public_bytes(serialization.Encoding.PEM))
        else:
            ca_crt_path = os.path.join(temp_dir, "empty_ca.pem")
            with open(ca_crt_path, "w", encoding="utf-8"):
                pass
