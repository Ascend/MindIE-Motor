...
def _uc_with_inline_ucm(storage, storage_backends, *, engine_key="motor_engine_prefill_config", multi=True):
    user_config = _uc(storage=storage)
    ucm_connector = {
    ...
    user_config[engine_key] = {"engine_config": {"kv_transfer_config": kv_config}}
    return user_config
...
def test_apply_storage_volumes_rejects_conflicting_template_volume():
    """A generated name must not silently reuse a template volume with another backend."""
    uc = _uc(storage=[{"type": "nfs", "server": "10.0.0.1", "path": "/kv", "mount_path": "/mnt/kv"}])
    pod_spec = {
        "volumes": [{"name": "mindie-motor-store-0", "persistentVolumeClaim": {"claimName": "unrelated-claim"}}]
    }

    with pytest.raises(ValueError, match="volume 'mindie-motor-store-0'.*conflicts"):
        apply_storage_volumes(pod_spec, {}, uc)


def test_apply_storage_volumes_rejects_conflicting_template_mount():
    """A generated name must not silently reuse a template mount with another path."""
    uc = _uc(storage=[{"type": "pvc", "claim_name": "shared", "mount_path": "/mnt/ucm"}])
    pod_spec = {"volumes": [{"name": "mindie-motor-store-0", "persistentVolumeClaim": {"claimName": "shared"}}]}
    container = {"volumeMounts": [{"name": "mindie-motor-store-0", "mountPath": "/mnt/wrong"}]}

    with pytest.raises(ValueError, match="volumeMount 'mindie-motor-store-0'.*conflicts"):
        apply_storage_volumes(pod_spec, container, uc)
...
@pytest.mark.parametrize("storage_backends", ["/mnt/ucm", ["/mnt/ucm"], "/mnt/ucm/"])
def test_inline_ucm_storage_backend_matches_enabled_mount(storage_backends):
    uc = _uc_with_inline_ucm(
        [{"type": "pvc", "storage_class_name": "nfs-sc", "mount_path": "/mnt/ucm"}],
        storage_backends,
    )
    assert len(get_storage_entries(uc)) == 1


def test_inline_ucm_storage_backend_matches_normalized_mount_path():
    """Normalize the storage mount as well as the UCM backend before comparing them."""
    uc = _uc_with_inline_ucm(
        [{"type": "pvc", "storage_class_name": "nfs-sc", "mount_path": "/mnt/ucm/"}],
        "/mnt/ucm",
    )
...
def test_inline_ucm_multiple_storage_backends_match_enabled_mounts():
    storage = [
        {"type": "pvc", "storage_class_name": "nfs-sc", "mount_path": "/mnt/ucm"},
        {"type": "nfs", "server": "10.0.0.1", "path": "/cache", "mount_path": "/mnt/cache"},
    ]
    uc = _uc_with_inline_ucm(storage, "/mnt/ucm:/mnt/cache")
    assert len(get_storage_entries(uc)) == 2


def test_inline_ucm_storage_backend_mismatch_rejected_before_yaml_generation():
    uc = _uc_with_inline_ucm(
        [{"type": "pvc", "storage_class_name": "nfs-sc", "mount_path": "/mnt/ucm"}],
        "/mnt/not-mounted",
    )
    with pytest.raises(ValueError, match="storage_backends.*mount_path"):
        get_storage_entries(uc)


def test_standalone_union_ucm_storage_backend_is_validated():
    uc = _uc_with_inline_ucm(
        [{"type": "hostpath", "path": "/data/ucm", "mount_path": "/mnt/ucm"}],
        "/mnt/not-mounted",
        engine_key="motor_engine_union_config",
        multi=False,
    )
    with pytest.raises(ValueError, match="storage_backends.*mount_path"):
        get_storage_entries(uc)
...
def test_file_based_ucm_config_skips_inline_backend_validation():
    uc = _uc(storage=[{"type": "pvc", "storage_class_name": "nfs-sc", "mount_path": "/mnt/ucm"}])
    uc["motor_engine_prefill_config"] = {
        "engine_config": {
            "kv_transfer_config": {
                "kv_connector": "UCMConnector",
                "kv_connector_extra_config": {"UCM_CONFIG_FILE": "/etc/ucm/config.yaml"},
            }
        }
    }
    assert len(get_storage_entries(uc)) == 1
...
def test_hostpath_rejects_paths_outside_operator_managed_roots(path):
    uc = _uc(storage=[{"type": "hostpath", "path": path, "mount_path": "/mnt/ucm"}])

    with pytest.raises(ValueError, match="absolute path under"):
        get_storage_entries(uc)


def test_hostpath_normalizes_allowed_source_path():
    uc = _uc(storage=[{"type": "hostpath", "path": "/mnt/cache/../ucm", "mount_path": "/mnt/ucm"}])
    pod_spec = {"volumes": []}

    apply_storage_volumes(pod_spec, {}, uc)

    assert pod_spec["volumes"][0]["hostPath"]["path"] == "/mnt/ucm"
